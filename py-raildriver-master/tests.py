...
class ListenerTestCase(AbstractRaildriverDllTestCase):
    ...
    def test_main_loop(self):
        with mock.patch.object(
            self.listener, "_main_iteration", side_effect=self.listener._main_iteration
        ) as mock_main_iteration:
            self.listener.start()
            time.sleep(0.3)
            self.listener.stop()
    ...
    def test_get_by_name_exists(self):
        with mock.patch.object(self.mock_dll, "GetControllerValue", return_value=0.5) as mock_gcv:
            with mock.patch.object(
                ...
                self.assertEqual(
                    self.raildriver.get_controller_value("Throttle", raildriver.VALUE_CURRENT),
                    0.5,
                )
                mock_gcv.assert_called_with(1, 0)
