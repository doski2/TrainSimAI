    ...
    for s in startup_vals:
        for h in hold_vals:
            for fall in fall_vals:
                ...
                cmd = [
                    'python', '-u', '-m', 'runtime.control_loop',
                    '--source', 'csv', '--run', str(RUN_FILE), '--mode', 'brake', '--rd', 'runtime.raildriver_stub:rd',
                    '--hz', str(hz), '--duration', str(duration), '--out', str(DATA.joinpath(f"run.ctrl_r{r}_s{s}_h{h}_f{fall}.csv")),
                    '--rise-per-s', str(r), '--fall-per-s', str(fall), '--startup-gate-s', str(s), '--hold-s', str(h),
                ]
